...
class PDFExtractorView:
    """Vista para el módulo de datos de estudiantes - Mismo diseño que módulo 2"""
    
    def __init__(self, root):
        self.root = root
        self.root.title("Extractor de Datos de Estudiantes")
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            main_frame,
            text="Extractor de Datos de Estudiantes",
            font=('Arial', 16, 'bold')
        )
        ...
        note_label = ttk.Label(
            main_frame,
            text="📚 Este módulo extrae nombres, DNI y nivel de riesgo de PDFs de estudiantes",
            font=('Arial', 10),
            foreground='#2E7D32'
        )
    ...
    def _create_preview_frame(self, parent):
        ...
        # Configurar columnas para datos de estudiantes
        self.data_tree['columns'] = ('nombres', 'dni', 'nivel_riesgo')
        
        self.data_tree.column('#0', width=60, minwidth=60, anchor=tk.CENTER)
        self.data_tree.column('nombres', width=350, minwidth=200)
        self.data_tree.column('dni', width=120, minwidth=100, anchor=tk.CENTER)
        self.data_tree.column('nivel_riesgo', width=150, minwidth=120, anchor=tk.CENTER)
        
        self.data_tree.heading('#0', text='#', anchor=tk.CENTER)
        self.data_tree.heading('nombres', text='Nombres y Apellidos')
        self.data_tree.heading('dni', text='DNI')
        self.data_tree.heading('nivel_riesgo', text='Nivel de Riesgo')
    ...
    def hide_loading(self):
        """Oculta el mensaje de carga de manera segura"""
        try:
            ...
            print(f"Error ocultando loading: {e}")
            self.loading_window = None
    ...
    def add_data_to_tree(self, index, nombres, dni, nivel_riesgo):
        """Agrega una fila de datos"""
        self.data_tree.insert(
            '', 
            tk.END, 
            text=str(index),
            values=(nombres, dni, nivel_riesgo)
        )
    ...
    def create_debug_window(self, pdf_name, text_content):
        """Crea ventana de debug con texto del PDF"""
        debug_window = tk.Toplevel(self.root)
        debug_window.title(f"Debug PDF: {pdf_name}")
        ...
        title = ttk.Label(
            frame,
            text=f"Texto extraído de: {pdf_name}",
            font=('Arial', 12, 'bold')
        )
        title.pack(pady=5)
